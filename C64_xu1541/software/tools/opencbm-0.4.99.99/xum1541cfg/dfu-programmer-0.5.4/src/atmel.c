                                   ...
                                   const uint8_t data1 )
{
    ...
    if( NULL != device ) {
        ...
        result = __atmel_read_page( device, data1, data1+1, (uint8_t *)buffer, false );
        if( 1 != result ) {
            return -5;
        }
                           ...
                           atmel_avr32_fuses_t *info )
{
    ...
    result = __atmel_read_page( device, 0, 32, (uint8_t *)buffer, false );
    if( 32 != result ) {
        return -5;
    }
    info->lock = 0;
...
#   define DM_AVR32 0x04
    ...
    for( i = 0; i < sizeof(data)/sizeof(atmel_read_config_t); i++ ) {
        ...
        {
            int16_t *ptr = row->offset + (int16_t *) info;
                    ...
                    const uint32_t end )
{
    ...
    /* Select USER page */
    if( 4 != dfu_download(device, 4, (uint8_t *)command) ) {
        DEBUG( "dfu_download failed.\n" );
        return -2;
    }
...
recheck_page:
        ...
        if( (0x10000U * (1 + mem_page)) <= last ) {
            if( first < (0x10000U * (1 + mem_page)) ) {
                last = 0x10000U * (1 + mem_page);
            } else {
                int32_t result;
